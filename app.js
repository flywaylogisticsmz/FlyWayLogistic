...
// ════════════════════════════════════════════════════
// GOOGLE SHEETS — URL
// Prioridade: 1) parâmetro ?gs= na URL  2) localStorage  3) vazio (modo local)
// ════════════════════════════════════════════════════
function getGsUrl() {
  const params  = new URLSearchParams(window.location.search);
  const fromUrl = params.get('gs');
  if (fromUrl && fromUrl.startsWith('http')) return fromUrl;
  return localStorage.getItem('fw_gs_url') || '';
}
...
function updateNavBadge() {
  const on = useSheets();
  document.getElementById('nav-dot').className  = 'dot ' + (on ? 'dot-on' : 'dot-off');
  document.getElementById('nav-lbl').textContent = on ? 'Google Sheets' : 'Sem ligação';
}
